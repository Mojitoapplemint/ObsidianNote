...
## Lemma : Is there languages and are not regular?
...
- Since $\sum^{*}$ is countab

## Definition
Let $L$ and $M$ be languages over alphabet $\sum$
5. Then $L\cup M$ is the language that contains all strings that are either or both in $L$ and $M$
6. Then $L\cap M$ is the language that contains all strings that are in both $L$ and $M$
7. Then $\bar{L}$, the complement of $L$, is the set of strings in $\sum^{*}$ that are not in $L$

## Identity of Regular Language
8. The amount of memory needed to determine whether a given string is in the language is finite and independent of the length of the string
9. 
