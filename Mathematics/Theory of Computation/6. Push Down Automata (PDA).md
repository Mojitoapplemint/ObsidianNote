...
Inductive Hypothesis: Assume all strings that brings $P$ to one state to other with empty stack in $k$ steps can be generated by $G$
...
- Since $y$ can bring $P$ from $s$ to $r$ without touching $u$, it can bring 
