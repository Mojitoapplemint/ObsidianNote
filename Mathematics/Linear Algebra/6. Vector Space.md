...
# Linear Combination
Let $\mathcal{V}$ a vector space over the field $\mathbb{F}$. Let a vector $v$ is a **Linear Combination** of vectors $\vec{u_{1}}, \vec{u_{2}}\dots \vec{u_{k}}\in\mathbb{R}^{n}$ if $\exists \text{ }a_{1}, a_{2}\dots a_{n}\in\mathbb{R}$ such that
