...
For axiom 4, since $\mathcal{V}$ is a vector space, $\exists \text{ }\vec{0}\in\mathcal{V}$ such that $\forall \text{ }\vec{v}\in\mathcal{V}$, $\vec{v}+\vec{0}=\vec{v}$
- Since $\forall \text{ }\vec{w}\in\mathcal{S}, \vec{w}\in \mathcal{V}$, thus $\vec{w}+\vec{0}=\vec{w}$
...
- Then since axiom 6 hold for $\mathcal{S}$, $-\vec{w}\in\mathcal{S}$
