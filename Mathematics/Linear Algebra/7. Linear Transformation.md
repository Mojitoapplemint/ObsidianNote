...
ii) $(\longleftarrow)$: Suppose $[T]_{D\leftarrow B}$ is invertible and $([T]_{D\leftarrow B})^{-1}=[T^{-1}]_{B\leftarrow D}$

Consider $([T]_{D\leftarrow B})^{-1}[T]_{D\leftarrow B}$
	$=[T^{-1}]_{B\leftarrow D}[T]_{D\leftarrow B}=[T^{-1}\circ T]_{B}$ by Theorem 3.2

Since $([T]_{D\leftarrow B})^{-1}$ and $[T]_{D\leftarrow B}$ are inverse,
	$=I$
	$=[I_{\mathcal{V}}]_{B}$

Therefore, $T^{-1}\circ T=I_{\mathcal{V}}$
- $T$ is invertible
