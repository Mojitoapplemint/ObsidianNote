...
$\mathcal{V}$ and $\mathbb{F}^{n}$ are the same
...
Suppose $\mathcal{V}$ and $\mathcal{W}$ are vector spaces over the same field and $\mathcal{V}$ is finite-dimensional. Then $\mathcal{V} \cong \mathcal{W}$ if and only if $\dim(\mathcal{W})$
