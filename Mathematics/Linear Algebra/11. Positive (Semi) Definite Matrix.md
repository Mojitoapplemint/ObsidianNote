...
## Corollary 5.7
A function $\langle \cdot, \cdot \rangle : \mathbb{F}^n \times \mathbb{F}^n \to \mathbb{F}$ is an inner product if and only if there exists an invertible matrix $P\in \mathcal{M_{n}(\mathbb{F})}$ such that
$$\langle \vec{v}, \vec{w} \rangle = \vec{v}^* P^{*}P \vec{w} \quad \text{for all} \quad \vec{v}, \vec{w} \in \mathbb{F}^n$$
**proof)**
From theorem 8.3, since $A$ is PD, there is an invertible matrix $P$ such that $P^{*}P=A$
