...
Suppose that $\mathcal{V}$ is an inner product space, $\mathbf{v} \in \mathcal{V}$ is a vector, and $c \in \mathbb{F}$ is a scalar. Then the following properties of the norm induced by the inner product hold:
1. Absolute Homogeneity : $\|\mathbf{cv}\| = |c|\|\mathbf{v}\|$, and  
2. $\|\mathbf{v}\| \geq 0$, with equality if and only if $\mathbf{v} = 0$.