...
Consider $|a_{n}-L|<\epsilon$. Since $a_{n}=\frac{n}{n+1}$ and $L=1$
$$|a_{n}-L|=\left| \frac{n}{n+1}-1 \right|=\left| \frac{n}{n+1}-\frac{n+1}{n+1} \right|=\left| \frac{n-(n+1)}{n+1} \right|=\left| \frac{-1}{n+1} \right|$$
Then, by the property of absolute value,
$$=|(-1)| \left| \frac{1}{n+1} \right|$$
Since $n>0$, $n+1>0$, thus $\frac{1}{n+1}>0$
$$=1\cdot \frac{1}{n+1}=\frac{1}{n+1}<\epsilon$$

Then, $\frac{1}{\epsilon}<n+1$. Thus, $\frac{1}{\epsilon}-1<n$

Let's try $K=\frac{1}{\epsilon}-1$

Let $\epsilon>0$, set $K=\frac{1}{\epsilon}-1$. 

Let $n>K$, and let's show $|a_{n}-L|<\epsilon$

 Since $a_{n}=\frac{n}{n+1}$ and $L=1$
$$|a_{n}-L|=\left| \frac{n}{n+1}-1 \right|=\left| \frac{n}{n+1}-\frac{n+1}{n+1} \right|=\left| \frac{n-(n+1)}{n+1} \right|=\left| \frac{-1}{n+1} \right|$$
Then, by the property of absolute value,
$$=|(-1)| \left| \frac{1}{n+1} \right|$$
Since $n>0$, $n+1>0$, thus $\frac{1}{n+1}>0$
$$=1\cdot \frac{1}{n+1}=\frac{1}{n+1}$$

Then, consider $n>K$. Then,
$$n+1>K+1$$
thus, 
$$\frac{1}{n+1}<\frac{1}{K+1}$$

Since $K=\frac{1}{\epsilon}-1$,
$$\frac{1}{K+1}=\frac{1}{\left(\frac{1}{\epsilon}+1\right)-1}$$
$$=\frac{1}{\frac{1}{\epsilon}}=\epsilon$$
Therefore, $|a_{n}-L|<\epsilon$ for all $n>K$

As a conclusion, according to the definition of convergence, this sequence is convergent to 1