...
If there are sequences $\{a_{n}\}, \{b_{n}\}, \{c_{n}\}$, with $\forall \text{ }n\in\mathbb{N}, a_{n}\leq b_{n} \leq c_{n}$ and there is a real number $L$ so that $\lim_{n\to \infty}{a_{n}}=\lim_{n\to \infty}{c_{n}}$
