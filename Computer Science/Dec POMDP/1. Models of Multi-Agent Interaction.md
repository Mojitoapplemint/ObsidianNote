...
- It is common to define observation function $\mathcal{O}_{i}$ for each agent $i$ that specifies probabilites over the agent's possible observations $o^{t}_{i}$ given the state $s^{t}$ and joint 